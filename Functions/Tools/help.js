...
async function HelpFuntion(interaction, client){
    ...
    collector.on('collect', async i => {
        if(i.customId === 'public'){
            ...
            return await i.update({ embeds: [embedPublic], components: [row(false, ButtonStyle.Secondary, ButtonStyle.Success, ButtonStyle.Success)] }).then(i => i.delete({ timeout: parseInt(5 * 1000)}));
        }
